...
<h1>SwiftTalon: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 SwiftTalon は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 SwiftTalon リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 皮皮虾，我们走！
...
### 🐜 革新的な省フットプリントデプロイ
SwiftTalon はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/sipeed/swifttalon/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに SwiftTalon を実行できます。
...
> [!TIP]
> `~/.swifttalon/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE で SwiftTalon と会話できます
...
> **Docker Compose**: `swifttalon-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。

</details>

## ⚙️ 設定

設定ファイル: `~/.swifttalon/config.json`

### ワークスペース構成

SwiftTalon は設定されたワークスペース（デフォルト: `~/.swifttalon/workspace`）にデータを保存します：
...
SwiftTalon はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.swifttalon/workspace` | エージェントの作業ディレクトリ |
...
SwiftTalon は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
| コマンド | 説明 |
|---------|------|
| `swifttalon onboard` | 設定＆ワークスペースの初期化 |
| `swifttalon agent -m "..."` | エージェントとチャット |
| `swifttalon agent` | インタラクティブチャットモード |
| `swifttalon gateway` | ゲートウェイを起動 |
| `swifttalon status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。SwiftTalon は手動検索用の便利なリンクを提供します。

Web 検索を有効にするには：
1. [https://brave.com/search/api](https://brave.com/search/api) で無料の API キーを取得（月 2000 クエリ無料）
2. `~/.swifttalon/config.json` に追加：
   ```json
   {
     "tools": {
       "web": {
...
別のインスタンスが実行中の場合に発生します。`swifttalon gateway` が 1 つだけ実行されていることを確認してください。
