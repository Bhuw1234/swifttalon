...
SwiftTalon 提供了一个现代化的终端用户界面 (TUI)，使用 Bubble Tea 框架构建，采用 **RAMBO 3D 战术风格**设计。
...
- **军事风格配色方案**:
  - `RamboRed (#FF2D2D)` - 攻击性红色，用于 Logo 和警告
  - `RamboOrange (#FF6B2D)` - 警告橙色，用于标题
  - `RamboGreen (#2DFF6B)` - 战术绿色，用于状态指示
  - `RamboCyan (#2DFFF0)` - 科技青色，用于模型选择器
  - `ElectricBlue (#2D7AFF)` - 电光蓝，用于边框

- **深色战术背景**:
  - `DarkBase (#0A0A0F)` - 近黑色基底
  - `DarkPanel (#12121A)` - 面板背景
  - `DarkSurface (#1A1A25)` - 表面色
