...
func Run(opts Options) (*Result, error) {
	...
	swiftTalonHome, err := resolveSwiftTalonHome(opts.SwiftTalonHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, swiftTalonHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to SwiftTalon")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, swiftTalonHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, swiftTalonHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(swiftTalonHome, "config.json"),
				Description: "convert OpenClaw config to SwiftTalon format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(swiftTalonHome)
...
func Execute(actions []Action, openclawHome, swiftTalonHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, swiftTalonHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, swiftTalonHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing SwiftTalon config: %w", err)
		}
...
func resolveSwiftTalonHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	...
	return filepath.Join(home, ".swifttalon"), nil
}
